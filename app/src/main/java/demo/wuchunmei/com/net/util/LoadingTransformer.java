...
/***
 * 用于RxJava的LoadDataAsyncTaskDialog进度提示弹窗
...
public class LoadingTransformer implements ObservableTransformer,DialogInterface.OnCancelListener {
    ...
    private LoadDataDialog mTaskDialog;
    ...
    private LoadDataDialog onCreateTaskDialog(Context context, String infoText) {
        if (context instanceof Activity) {
            return new LoadDataDialog(context,infoText);
        }
        return null;
    }
    ...
    private void dismissTaskDialog() {
        if (mTaskDialog != null) {
            ...
            WindowManager.LayoutParams lp = ((Activity)mContextRef.get()).getWindow().getAttributes();
            lp.alpha = 1f;
            ((Activity)mContextRef.get()).getWindow().setAttributes(lp);
        }
    }
